...
<template>
  <div class="badge" :style="{ borderColor: color, color }" title="Leadership readiness">
    <div class="row">
...
const focus = computed(() => {
  ...
  const picks = plan.slice(0, 2).map(p => p?.skill).filter(Boolean)
...
<style scoped>
.badge { border: 2px solid; border-radius: 12px; padding: 10px 12px; }
